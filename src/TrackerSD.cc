...
void TrackerSD::EndOfEvent(G4HCofThisEvent*)
{
	...
	std::ofstream fout;
  fout.open("process_run8.txt", std::fstream::app);
  fout <<cname<< " " <<std::endl;;
  fout.close();
fout.open("alphadata100MeV_run8.txt", std::fstream::app);
	fout <<name<< " "<<std::endl;;
  fout.close();
  fout.open("number100MeV_run8.txt", std::fstream::app);
	fout <<NbHits<< " "<<std::endl;;
  fout.close();
  fout.open("energydata100MeV_run8.txt", std::fstream::app);

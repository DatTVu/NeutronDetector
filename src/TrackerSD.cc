...
void TrackerSD::EndOfEvent(G4HCofThisEvent*)
{
	...
	std::ofstream fout;
  fout.open("process.txt", std::fstream::app);
  fout <<cname<< " " <<std::endl;;
  fout.close();
/*fout.open("alphadata200MeV.txt", std::fstream::app);
	fout <<name<< " "<<std::endl;;
  fout.close();
  fout.open("number200MeV.txt", std::fstream::app);
	fout <<NbHits<< " "<<std::endl;;
  fout.close();
  fout.open("energydata200MeV.txt", std::fstream::app);
